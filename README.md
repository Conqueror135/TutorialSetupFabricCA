...
1.6. Đăng ký identity cho admin TLS CA
    ...
    ***Các thư mục này được ứng dụng khách Fabric CA sử dụng để:***
    ...
    1.6.1. Sao chép tệp chứng chỉ gốc của TLS CA tls_ca/ca-cert.pem, được tạo khi máy chủ TLS CA được khởi động, sang ca_client/tls-root-cert /tls-ca-cert.pem. Lưu ý rằng tên tệp được thay đổi thành tls-ca-cert.pem để làm rõ đây là chứng chỉ gốc từ TLS CA. 
    
    ***Note***: Chứng chỉ gốc TLS CA này sẽ cần có sẵn trên mỗi hệ thống sử dụng fabric-ca-client  để tương tác với server TLS CA

    1.6.2. Máy khách Fabric CA cũng cần biết tệp nhị phân máy khách Fabric CA nằm ở đâu. Biến môi trường FABRIC_CA_CLIENT_HOME được sử dụng để đặt vị trí: `export FABRIC_CA_CLIENT_HOME=<FULLY-QUALIFIED-PATH-TO-FABRIC-CA-BINARY>`
    ...
    1.6.3. Sử dụng CLI ứng dụng khách Fabric CA để đăng ký danh tính quản trị viên bootstrap TLS CA. Chạy lệnh:
    ```
...
2.1. Đăng ký định danh TLS cho Organization CA
...
2.3. Enroll tài khoản admin và đăng ký danh tính người dùng cho tổ chức CA vừa tạo
    
    ***Note:*** Các thao tác tương tác với CA thông qua fabric-ca-client và thực hiện tại terminal tong thư mục ca_client

    2.3.1. Cấp danh tính cho tài khoản admin của Organization CA
    ...
    ***Trong hướng dẫn này thì câu lệnh sẽ như sau:***
    ...
    2.3.2. Cấp danh tính cho một người dùng trong tổ chức
...
## III. Hướng dẫn cho việc chạy thử CA mẫu đã tạo ở các phần trước

    - Đầu tiên clone repository này về máy
    - Đối với thử nghiệm tạo thêm một tổ chức CA mới thì trước hết cần vào thư mục tls_ca và chạy câu lệnh `./fabric-ca-server start` để khởi động máy chủ TLS CA. Sau đó thực hiện việc tạo tổ chức CA mới như hướng dẫn trong phần II.
    - Đối với thử nghiệm đối với ca_org1 đã tạo sẳn thì chỉ cần vào thư mục ca_org1 và chạy câu lệnh `./fabric-ca-server start` để khởi động máy chủ CA của org1. Sau đó có thể thực hiện việc đăng ký cấp phát danh tính cho một người dùng trong tổ chức org1 như hướng dẫn trong mục 2.3.2 của phần II.

    ***Note:*** Trước khi thực hiện các thao tác tương tác với CA qua fabric-ca-client ở thư mục ca_client thì cần phải export biến môi trường FABRIC_CA_CLIENT_HOME như hướng dẫn trong mục 1.6.2 của phần I.